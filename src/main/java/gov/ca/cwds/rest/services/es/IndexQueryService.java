...
public class IndexQueryService extends
   ...
   */
  protected String callDao(final String auth, final String index, final String query) {

    return this.elasticsearchDao.searchIndexByQuery(auth, index, query);
  }
  ...
  protected IndexQueryResponse handleRequest(IndexQueryRequest req) {
    ...
    return new IndexQueryResponse(callDao(req.getAuth(), req.getIndex(), query));
  }

  @Override
  protected IndexQueryResponse handleFind(String searchForThis) {
    try {
      return new IndexQueryResponse(callDao("auth",
